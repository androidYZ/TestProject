...
public class BindViewProcessor extends AbstractProcessor {
    ...
    private String generateCode(TypeElement typeElement) {
        ...
        builder.append("import com.example.aptapi.template.IBindHelper;\n\n"); //构建import类的代码
        ...
        for (ViewInfo viewInfo : mToBindMap.get(typeElement)) { //遍历每一个需要绑定的view
            ...
            builder.append(rawClassName + " substitute"+viewInfo.id+" = " + "(" + rawClassName + ")" + "target;\n");    //强制类型转换

            builder.append("\t\t"); //代码格式，可以忽略
            builder.append("substitute"+viewInfo.id+"." + viewInfo.viewName).append(" = ");    //构建赋值表达式
            builder.append("substitute"+viewInfo.id+".findViewById(" + viewInfo.id + ");\n");  //构建赋值表达式
        }
        builder.append("\t}\n");    //代码格式，可以忽略
        builder.append('\n');   //代码格式，可以忽略
        builder.append("}\n");  //代码格式，可以忽略
